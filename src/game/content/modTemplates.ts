...
const MVP_MODS: Record<SpecModId, ModTemplate> = {
  ...
  'mod-poison-up': {
    id: 'mod-poison-up',
    label: 'Концентрированный яд',
    emoji: '💧',
    group: 'damage',
    tags: ['dot'],
    requires: ['dot'],
    enabled: false,
    descriptionLines: ['+50% к урону яда. Сила растёт с Lm. (фаза 2).'],
    ops: [{ kind: 'damage_mult', base: 0.5, scaleMode: 'percent' }],
  },
  'mod-regen-up': {
    id: 'mod-regen-up',
    label: 'Усиленная регенерация',
    emoji: '💙',
    group: 'survival',
    tags: ['regen'],
    requires: ['regen'],
    enabled: false,
    descriptionLines: ['+50% к HoT. Сила растёт с Lm. (фаза 2).'],
    ops: [{ kind: 'heal_mult', base: 0.5, scaleMode: 'percent' }],
  },
  'mod-buff-duration': {
    id: 'mod-buff-duration',
    label: 'Длительный эффект',
    emoji: '⏳',
    group: 'utility',
    tags: ['buff'],
    requires: ['buff'],
    enabled: false,
    descriptionLines: ['+1 ход к длительности баффа. Сила растёт с Lm. (фаза 2).'],
    ops: [{ kind: 'cooldown_add', base: 1, scaleMode: 'flat' }],
  },
  'mod-resurrect-hp': {
    id: 'mod-resurrect-hp',
    label: 'Возвращение к жизни',
    emoji: '✨',
    group: 'survival',
    tags: ['resurrect'],
    requires: ['resurrect'],
    enabled: false,
    descriptionLines: ['+10% maxHp при воскрешении. Сила растёт с Lm. (фаза 2).'],
    ops: [{ kind: 'heal_mult', base: 0.1, scaleMode: 'percent' }],
  },
  'mod-debuff-range': {
    id: 'mod-debuff-range',
    label: 'Широкий дебафф',
    emoji: '🌀',
    group: 'utility',
    tags: ['debuff'],
    requires: ['debuff'],
    enabled: false,
    descriptionLines: ['+1 к радиусу дебаффа. Сила растёт с Lm. (фаза 2).'],
    ops: [{ kind: 'range_add', base: 1, scaleMode: 'flat' }],
  },
}
